...
class CEquipmentCommandWindow:
	public C2DWindow
{
	static _TCHAR				m_CommandName[EQUIPMENT_COMMAND_MAX][FONT_MAX];			//コマンド名
	static LPD3DXFONT			m_Font;											//描画用フォント
	static RECT					m_Pos[EQUIPMENT_COMMAND_MAX];								//ウィンドウ表示位置

	static bool					m_bDrawFlg;										//描画フラグ

	static VERTEX_2D			m_aVertex[EQUIPMENT_COMMAND_MAX][NUM_VERTEX];				//ポリゴン頂点情報格納用

	static CTurn				m_Turn;											//ステート変更

	static CEquipmentInventory	m_Inbentory;									//インベントリの描画フラグ変更用
