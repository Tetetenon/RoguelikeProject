...
//静的メンバ変数定義
//---------------------------------------------------------------------------------------
CHierarchieNum* CHierarchieNum::m_pHierarchieNum = NULL;
...
//実体の作成
//---------------------------------------------------------------------------------------
void CHierarchieNum::Create()
{
	//中身がなければ作成
	if (!m_pHierarchieNum)
	{
		m_pHierarchieNum = new CHierarchieNum;
	}
}
//---------------------------------------------------------------------------------------
//実体の削除
//---------------------------------------------------------------------------------------
void CHierarchieNum::Delete()
{
	//中身があれば削除
	if (m_pHierarchieNum)
	{
		delete m_pHierarchieNum;
		m_pHierarchieNum = NULL;
	}
}
//---------------------------------------------------------------------------------------
//実体のポインタを渡す
//---------------------------------------------------------------------------------------
CHierarchieNum* CHierarchieNum::GetPointer(void)
{
	//念のため作成関数を呼ぶ
	Create();
	return m_pHierarchieNum;
}
//---------------------------------------------------------------------------------------
//メンバ変数のポインタをセットする
//---------------------------------------------------------------------------------------
void CHierarchieNum::SetPointer()
{
	//ポインタの取得
...
{
	//階層数を保存
	int HierarchieNum = m_pMapData->GetHierarchieNum();
	...
	for(int i =0 ;i < HIERARCHIENUM;i++)
	{
		...
		//アルファ値を設定
		int AlphaNum = 255 - m_pFade->GetFadeAlpha();
