...
// -------------------------------------------------------------------
LRESULT CALLBACK CWindow::WndMapProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)	//UINT型:4バイトの符号なし整数
{
...
// ウィンドウ生成
// ---------------------------------------------------------------------
bool CWindow::Create(LPCTSTR pszWindowName,		//ウインドウの名前
	DWORD dwStyle,								//ウインドウのスタイル
	DWORD dwExStyle,							//ウインドウの拡張スタイル
	int nX, int nY,								//ウインドウの初期位置
	int nWidth, int nHeight,					//ウインドウの高さ、幅
	HWND hwndParent,							//親ウインドウのハンドル(今回はNULLのためこれが最上位ウインドウとなる)
	HMENU nIDorHMenu,							//メニューのスタイルに応じて、子ウインドウ、メニューのIDを取得する(今回はNULLのため、子ウインドウ、メニューを持たない)
	LPVOID pParam)								//メッセージでlParamパラメータに渡される構造体によりウィンドウに渡される値へのポインタを指定(今回はNULLのため使用しない)
{
	//ウインドウクラスの登録
	if (!RegisterClass())
		return false;
	...
	HWND hWnd = ::CreateWindowEx(
		...
		m_pszClassName,				//登録されているクラス名
		pszWindowName,				//ウインドウ名
		dwStyle,					//ウインドウのスタイル
		nX, nY,						//ウインドウの初期位置
		nWidth, nHeight,			//ウィンドウの縦幅、横幅
		hwndParent,					//親ウインドウのハンドル(今回は使用しない)
		nIDorHMenu,					//メニューハンドル、子識別子
	...
	HWND hwndParent, HMENU nIDorHMenu, LPVOID pParam)
{
	...
	if (!::AdjustWindowRectEx(&rc,						//クライアント(ウインドウの)領域座標が入った構造体へのポインタ
		dwStyle,										//ウインドウのスタイル
		nIDorHMenu && !IS_INTRESOURCE(nIDorHMenu),		//メニューを持つかどうかの指定(nIDorHMenuはNULLが入っている(たぶん)ので、持たない(はず))
		dwExStyle))										//拡張ウインドウスタイル(今回の場合、値が0のため、おそらく指定しない)
		return false;
...
bool CWindow::Attach(HWND hWnd)
{
	...
	m_hWnd = hWnd;
	// ダイアログかウィンドウかを判定する
	m_bDialog = (::GetWindowLong(hWnd, DWL_DLGPROC) != 0);	//GetWindowLong(hWnd,DWL_DLGPROC)ウインドウのハンドル、取得する値のオフセット
	int nIndex = m_bDialog ? DWL_DLGPROC : GWL_WNDPROC;

	// ウィンドウハンドルと CWindow オブジェクトを結びつける
	::SetProp(m_hWnd,	//ウインドウのハンドル
		g_szProc,		//文字列
		(HANDLE)this);	//データのハンドル(this:自分自身を表すポインタ)

	// 既存のウィンドウをサブクラス化する場合は、ウィンドウ(ダイアログ)
	// プロシージャをWndMapProcに置き換える
	//GetWindowLong(ウインドウのハンドル,取得するオフセット)	:指定されたウインドウの情報を取得する。
